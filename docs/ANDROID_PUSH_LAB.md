...
7. Disable through Profile: Android **immediately** clears local permission for this account and disables Firebase token auto-initialization, then attempts authenticated DELETE to revoke the server registration. If offline, the client refuses all incoming data-only notifications locally, saves a session-scoped pending server revoke and exposes **Повторить отключение** on Profile. Remote removal is not yet confirmed while offline; use that button when connected. If Android permission is removed in Settings, the service independently refuses to post notifications.
