...
- The voice button requests `RECORD_AUDIO` permission only after an explicit tap. The user can stop or cancel recording. The recorder uses AAC audio in an MP4 (`.m4a`) file. Unsent recordings live only in the app cache and are deleted when dismissed or when leaving the chat.
...
- Incoming attachment messages show a button which retrieves an authenticated short-lived read URL; recorded audio is played in-app through `MediaPlayer`. Photo/video currently use the system `ACTION_VIEW` handler.
...
3. `ACTION_VIEW` gives an external viewer a time-limited signed URL. External apps may retain these URLs during their validity; before production, use an in-app viewer or a secure temporary local cache with a revocable content URI.
