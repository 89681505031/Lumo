...
This tracker describes what exists in the stacked development branches. **None of the staging-only server capabilities should be described as production-ready until their own security/device/deployment gates pass.** The latest integration branch is `feature/lumo-media-retention-quota`, adding bounded private-media reservations and secret-protected abandoned-upload cleanup on top of pagination, privacy push and encrypted group offline state.
...
| 5 | Photo/video/files | Private photo/video/voice plus PDF/TXT/DOCX/XLSX/PPTX staging attachments for direct chats **and private groups**, authenticated short-lived signed downloads, per-account outstanding upload quota (8 / 75 MiB), and cron-protected cleanup of expired **unclaimed** objects. | Dedicated staging bucket/CORS, malware/quarantine pipeline, long-term claimed-media retention/storage quotas, monitoring and multi-device QA before broad production rollout. |
...
| 10 | Groups | Cosmic private group UI plus PostgreSQL membership/roles, removal/rejoin privacy, idempotent sends, WebSocket + polling fallback, replies/search/actions, private attachments, stable pagination, generic privacy-preserving group push, and shared abandoned-upload quota/cleanup. | Multi-device concurrency/scroll/push QA, moderation/abuse controls, claimed-media retention cleanup and E2E work. |
