...
This tracker describes what exists in the stacked development branches. **None of the staging-only server capabilities should be described as production-ready until their own security/device/deployment gates pass.** The latest integration branch is `feature/lumo-private-push-current-stack`, continuing the stacked feature chain through group pagination plus privacy-first direct/group push staging.
...
| 8 | Notifications | Privacy-first FCM debug/staging path now connected to the current backend for direct **and private-group** messages: explicit session-scoped opt-in, content-free data payloads, read/deletion/block/membership rechecks, bounded outbox leases/retries and offline revoke reconciliation. | Dedicated staging Firebase/backend/cron credentials, physical-device permission/Doze/token/multi-instance tests. Release remains no-op. |
