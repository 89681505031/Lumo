...
This tracker describes what exists in the stacked development branches. **None of the staging-only server capabilities should be described as production-ready until their own security/device/deployment gates pass.** The latest integration branch is `feature/lumo-group-replies-search`, continuing the stacked feature chain through durable private groups plus membership-safe linked replies and search.
...
| 10 | Groups | Cosmic private group UI plus PostgreSQL creation/membership/roles, removal/rejoin privacy, idempotent text sends, same-instance WebSocket + polling fallback, membership-window-safe linked replies and scoped in-group search. | Multi-device QA, pagination, moderation/abuse controls, group push, reactions/edit-delete and private group attachments. |
