...
This tracker describes what exists in the stacked development branches. **None of the staging-only server capabilities should be described as production-ready until their own security/device/deployment gates pass.** The latest integration branch is `feature/lumo-current-stack-offline`, combining stable group pagination, privacy-first generic direct/group push staging, and encrypted offline private-group history/retry state.
