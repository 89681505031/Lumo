...
export const postgresStore = {
  ...
  async registerPushDevice({userId,sessionToken,tokenHash,token}) {
    // One physical FCM token may move between accounts on the same phone.
    // Serialize by hash so concurrent server instances cannot leave it bound
    // to two Lumo sessions.
    ...
    try {
      ...
      await client.query(`
        insert into push_devices(session_token,user_id,token_hash,fcm_token)
        values($1,$2,$3,$4)
