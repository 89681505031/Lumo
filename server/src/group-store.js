...
export const groupStore = {
  ...
  async historyPage(userId,groupId,{beforeCreatedAt=null,beforeId=null,limit=50}={}) {
    const membership=await this.membership(userId,groupId);
    if(!membership)return null;
    const rows=await dbQuery(`
      select gm.*,
      ...
      where m.group_id=$1 and m.user_id=$2
        and (
          ...
          or (gm.created_at,gm.id)<($3::timestamptz,$4::uuid)
        )
      order by gm.created_at desc,gm.id desc
      limit $5`,
      [groupId,userId,beforeCreatedAt,beforeId,limit+1]
    );
    ...
    return {
      ...
      next:hasMore&&oldest ? {createdAt:iso(oldest.created_at),id:oldest.id} : null
    };
  },
